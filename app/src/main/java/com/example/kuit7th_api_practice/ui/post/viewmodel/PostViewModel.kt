...
class PostViewModel @Inject constructor(
    ...
    // 게시글 목록 UI 상태 (PostListScreen 에서 viewModel.UiState 로 참조)
    private val _uiState = MutableStateFlow<PostUiState>(PostUiState.Idle)
    val UiState: StateFlow<PostUiState> = _uiState.asStateFlow()

    // 상세 화면 UI 상태 — StateFlow 로 관리
    ...
    fun fetchPosts(userId: Int? = null) {
        ...
        viewModelScope.launch {
            postRepository.getPosts(userId)
                ...
                .collect { posts -> _uiState.value = PostUiState.Success(posts) }
        }
    }
    ...
    fun getPostDetail(postId: Long) {
        ...
        // getPost 는 Flow<Post> 를 반환하므로 fetchPosts 와 같은 방식으로 collect
        viewModelScope.launch {
            runCatching {
                postRepository.getPost(postId.toInt())
                    .collect { post -> showPostDetail(post) }
            }.onFailure { error ->
    ...
    private fun addOrReplacePostInList(post: Post) {
        ...
        val posts = currentState.posts.filterNot { it.id == post.id }
